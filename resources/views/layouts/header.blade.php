...
<nav class="navbar  site-wrap navbar-expand-md navbar-light sticky-top" style="z-index:3;   background-color: #fff;">
    <div class="container-fluid">
        <a class="navbar-brand mb-0 p-0" style="color:#000;   font-size: 15px;" href="{{ route('index') }}"><img
                class="mr-3" src="{{ $configuration->getMedia('logo')->first()->getFullUrl() }}"
                alt="{{ $configuration->name }}">{{ $configuration->name }}</a>
        ...
        <div class="collapse navbar-collapse" id="collapsibleNavbar">
            <ul class="navbar-nav ml-auto text-dark" style="color: #000;">
                ...
                <li class="nav-item  {{ $pagename == 'index' ? 'active' : '' }}"><a href="{{ route('index') }}"
                        class="nav-link">Home</a></li>
                <li class="nav-item {{ $pagename == 'about' ? 'active' : '' }}"><a href="{{ route('about') }}"
                        class="nav-link">About</a></li>
                {{-- <li class="nav-item  {{ $pagename=="contact"?"active":"" }}"><a href="{{ route('contact') }}" class="nav-link">Contact</a></li> --}}
                <li class="nav-item  {{ $pagename == 'nuggets' ? 'active' : '' }}"><a href="{{ route('nuggetquote') }}"
                        class="nav-link">Nugget</a></li>
                <li class="nav-item  {{ $pagename == 'blogs' ? 'active' : '' }}"><a href="{{ route('ourblogs') }}"
                    ...
                    <li class="nav-item dropdown {{ $pagename == 'dashboard' ? 'active' : '' }}">
                        ...
                        <a id="navbarDropdown" class="nav-link dropdown-toggle" href="{{ $url }}" role="button"
                            ...
                            <img src="{{ Auth::user()->getMedia('avatar')->first()->getFullUrl('avatar') }}"
                                class="rounded-circle" alt="{{ ucwords(Auth::user()->name) }}">
                        </a>
            ...
            <div class="row align-items justify-content-center">
                ...
                <div class="row ">
                    <div class="col-md-9 bottomleft ">
                        <p class="breadcrumbs mt-2"><span class="mr-2"><a href="{{ route('index') }}"
                                    class="text-white">Home <i class="fa fa-chevron-right"></i></a></span>
                            <span>{!! $pagename == 'index' ? '' : ucwords(str_replace('%20', ' ', $pagename)) !!}</ <i class="fa fa-chevron-right"></i></span></p>
